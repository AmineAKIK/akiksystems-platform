import { parseSystemPublicationSnapshot } from '@akiksystems/db';
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const publication = await appDb
    ...
    .select(['system_publications.snapshot'])
    .where('systems.lifecycle', '=', 'active')
    .where('system_publications.locale', '=', locale)
    .where('system_publications.slug', '=', slug)
    .executeTakeFirst();

  const snapshot =
    publication === undefined
      ? null
      : parseSystemPublicationSnapshot(publication.snapshot);
  const publishedAsset = snapshot?.media.find((asset) => asset.id === assetId);
